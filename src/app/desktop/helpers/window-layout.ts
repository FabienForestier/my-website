...
export function computeLayout(): Record<string, { x: number; y: number; w: number; h: number }> {
  ...
  return {
    ...
    reco: {
      x: Math.round(w / 2 - 230),
      y: Math.max(TOP + 40, Math.round(h / 2 - 230)),
      w: 460,
      h: Math.min(460, usableH),
    },
    terminal: {
      x: Math.round(w / 2 - 290),
      y: Math.max(TOP + 60, Math.round(h / 2 - 200)),
      w: 580,
      h: 380,
    },
    education: {
      x: Math.round(w / 2 - 220),
      y: Math.max(TOP + 80, Math.round(h / 2 - 160)),
      w: 440,
      h: 320,
    },
  };
